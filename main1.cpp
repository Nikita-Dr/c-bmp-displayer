...
public:
    ...
    void OpenBmp(std::string filePath)
    {
        // открываем файл
    ...
    void DisplayBmp()
    {
        for (int i = fileInfoHeader.biHeight - 1; i > 0; i--) {
                for (unsigned int j = 0; j < fileInfoHeader.biWidth; j++) {
                    if (rgbInfo[i][j].rgbRed + rgbInfo[i][j].rgbGreen + rgbInfo[i][j].rgbBlue > 0){
                        std::cout << "#";
                    } else std::cout << " ";      
                }
                std::cout << std::endl;
...
int main(int argc, char *argv[])
{
    ...
    BmpImage bmpImage;
    std::cout << "Opeing: " << filePath << std::endl;
    bmpImage.OpenBmp(filePath);
    std::cout << "Displaying: " << filePath << std::endl;
    bmpImage.DisplayBmp();
